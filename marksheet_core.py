...
Handling of non-numeric marks:
- If a student's Marks cell contains letters/words (e.g. "Absent", "Malpractice")
  or is blank, that value is carried through as-is into the report (with its
  first letter capitalised) instead of being treated as an error. Such rows
  show the same remark in the "out of 25" and "SA" columns too, since no
  numeric score can be derived from a remark.

Conditional 4th column:
- The raw "Marks (out of <Maximum Test Marks>)" column is only included in
  the generated report when Maximum Test Marks is NOT 25 - if it IS 25, that
  column would be identical to the "Marks (out of 25)" column, so it is
...
def _format_remark(text: str) -> str:
    """
    Formats a non-numeric marks entry (a word, phrase, or blank) for display:
    capitalises only the first letter, leaves the rest exactly as entered,
    and returns an empty string for blank/whitespace-only entries.
    """
    text = text.strip()
    if not text:
        return ""
...
def parse_marks_docx(file) -> pd.DataFrame:
    ...
    The Marks column may contain a number, OR a non-numeric remark such as
    "Absent" or "Malpractice", OR be blank. Non-numeric/blank entries are
    kept as text (first letter capitalised) rather than raising an error.

    Returns a DataFrame with columns: Sl.No, Name, APAAR ID, Marks
    (Marks holds either a float or a remark string).
    """
    try:
        doc = Document(file)
    ...
    for r in rows[1:]:
        ...
        try:
            ...
            # Non-numeric (e.g. "Absent") or blank - keep as a formatted remark.
            marks_val = _format_remark(raw_marks)
        records.append(
            {
...
def compute_marks(df: pd.DataFrame, max_test_marks: float, sa_component: float) -> pd.DataFrame:
    ...
    Rows where Marks is a non-numeric remark (e.g. "Absent") or blank are
    left as-is: the same remark is carried into Marks_25 and SA instead of
    a computed number, since no numeric score exists for that student.
    """
    if max_test_marks <= 0:
    ...
    def scale_25(v):
        if isinstance(v, (int, float)):
            return round(v / max_test_marks * 25, 1)
        return v  # remark or blank - carried through unchanged

    def scale_sa(v25):
        if isinstance(v25, (int, float)):
            return round(v25 / 25 * sa_component, 1)
        return v25  # remark or blank - carried through unchanged

    out["Marks_25"] = out["Marks"].apply(scale_25)
    out["SA"] = out["Marks_25"].apply(scale_sa)
    return out
...
def _fmt_cell(v):
    """Formats a Marks/Marks_25/SA value for display: 1 decimal for numbers, as-is for remarks."""
    if isinstance(v, (int, float)):
        return f"{v:.1f}"
    return v if v else ""
...
def generate_marksheet(
    ...
    Columns are: Sl.No | Name | APAAR ID | [Marks (out of Maximum Test Marks)] |
    Marks (out of 25) | SA (out of SA Component)
    The bracketed raw-marks column is included only when Maximum Test Marks
    is not 25 (when it IS 25, that column would just duplicate the "out of 25"
    column, so it's left out automatically).
    """
    doc = Document()
    ...
    # --- Marks table ---
    include_raw_col = not math.isclose(max_test_marks, 25, abs_tol=1e-6)

    if include_raw_col:
        num_cols = 6
        widths = [Inches(0.6), Inches(1.6), Inches(1.3), Inches(0.9), Inches(1.1), Inches(1.1)]
        headers = [
            "Sl.No",
            "Name",
            "APAAR ID",
            f"Marks\n(out of {max_test_marks:g})",
            "Marks\n(out of 25)",
            f"SA\n(out of {sa_component:g})",
        ]
    else:
        num_cols = 5
        widths = [Inches(0.7), Inches(1.9), Inches(1.5), Inches(1.3), Inches(1.3)]
        headers = [
            "Sl.No",
            "Name",
            "APAAR ID",
    ...
    marks_table = doc.add_table(rows=1, cols=num_cols)
    ...
    for _, row in df.iterrows():
        ...
        for i, val in enumerate(values):
            ...
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT if i in left_align_idx else WD_ALIGN_PARAGRAPH.CENTER
            p.add_run(val)
